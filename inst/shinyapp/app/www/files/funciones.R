...
replace_country <- function(c_text){
  ...
  while(continue){
    ...
    else if (str_detect(texto, "colombia")) {
      c_text[i] <- "colombia"
      i = i + 1
    } 
    ...
    else if (str_detect(texto, "usa") || str_detect(texto,"centennial") || str_detect(texto, "texas") || 
    ...
    else if(str_detect(texto, "italy") || texto == "genova italy" || texto == "italy" || texto == "treviso italy"
